...
PageDirBase	equ	200000h		; 页目录起始物理地址 (4K 对齐)
PageTblBase	equ	201000h		; 页表起始物理地址 (4K 对齐)
...
szPaging:	db 'setup paging...', 0
Offset_szPaging	equ szPaging - LABEL_SEG_DATA

szCR0:		db 'CR0:', 0
Offset_szCR0	equ szCR0 - LABEL_SEG_DATA

szCR3:		db 'CR3:', 0
Offset_szCR3	equ szCR3 - LABEL_SEG_DATA
...
;-----------测试分页机制-----------
	...
	; 显示 'setup paging...', cr0 和 cr3 的内容
	mov	edi, (80 * 14 + 0) * 2	; 屏幕第 14 行, 第 0 列
	mov	esi, Offset_szPaging
	call	DispStr
	...
	mov	eax, cr0
	call	Disp_DWORD
	mov	edi, (80 * 16 + 0) * 2	; 屏幕第 16 行, 第 0 列
	mov	esi, Offset_szCR3
	call	DispStr
	mov	eax, cr3
	call	Disp_DWORD

...
PageDirBase0	equ	200000h		; 页目录起始物理地址 2M
PageTblBase0	equ	201000h		; 页表起始物理地址 2M + 4K
PageDirBase1	equ	601000h		; 页目录起始物理地址 2M + 4K + 4M
PageTblBase1	equ	602000h		; 页表起始物理地址 2M + 4K + 4M + 4K

LinearAddrDemo	equ	1002000h
ProcFoo		equ	1002000h
ProcBar		equ	1003000h
ProcPagingDemo	equ	1004000h
...
LABEL_DESC_PAGE_DIR0:	Descriptor	PageDirBase0,	4095,		DA_D32		; 页目录描述符
LABEL_DESC_PAGE_TBL0:	Descriptor	PageTblBase0,	1023,		DA_D32|DA_G_4K	; 页表描述符 (4K 粒度)
LABEL_DESC_PAGE_DIR1:	Descriptor	PageDirBase1,	4095,		DA_D32
LABEL_DESC_PAGE_TBL1:	Descriptor	PageTblBase1,	1023,		DA_D32|DA_G_4K
...
Selector_PageDir0	equ	LABEL_DESC_PAGE_DIR0	- LABEL_GDT
Selector_PageTbl0	equ	LABEL_DESC_PAGE_TBL0	- LABEL_GDT
Selector_PageDir1	equ	LABEL_DESC_PAGE_DIR1	- LABEL_GDT
Selector_PageTbl1	equ	LABEL_DESC_PAGE_TBL1	- LABEL_GDT
...
SetupPaging:
	...
	mov	ax, Selector_PageDir0
	mov	es, ax
	xor	edi, edi		; es:edi -> 页目录的物理基地址
	xor	eax, eax
	mov	eax, PageTblBase0 | PG_P | PG_RWW | PG_US1
...
PageSwitch:
	; 初始化页目录
	mov	ax, Selector_PageDir1
	mov	es, ax
	xor	edi, edi
	xor	eax, eax
	mov	eax, PageTblBase1 | PG_P | PG_RWW | PG_US1
	mov	ecx, 1024
	cld
.init_pagedir1:
	stosd
...
.init_pagetbl1:
	...
	mul	ebx
	add	eax, ecx		; (EAX)=页表项相对 PageTblBase1 的偏移
...
LABEL_SEG_CODE3:
	...
	mov	ecx, 5			; 打印 5 个栈单元
	call	PrintStack_Ring3

	; 使用调用门，特权级转移 ring3 -> ring0
